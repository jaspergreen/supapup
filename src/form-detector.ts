...
export class FormDetector {
  ...
  /**
   * Find all form containers (actual forms or divs with form fields)
   */
  private static findFormContainers(): Element[] {
    ...
    // Find divs/sections that contain multiple form fields but aren't in a <form>
    const potentialContainers = document.querySelectorAll('div, section, article');
    potentialContainers.forEach(container => {
      const inputs = container.querySelectorAll('input, textarea, select');
      const forms = container.querySelectorAll('form');
      
      // If it has inputs but no nested forms, and at least 3 fields, consider it a form
      if (inputs.length >= 3 && forms.length === 0) {
        // Check if inputs are direct descendants (not deeply nested)
