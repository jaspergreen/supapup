...
export class NetworkTools {
  ...
  private async setupNetworkMonitoring() {
    ...
    // Add page error monitoring for JavaScript errors including throw new Error()
      ...
      content: [
        {
          ...
          text: `📋 Console Logs (${recentLogs.length} entries)\n\n` +
                recentLogs.map((log, i) => {
                  let entry = `[${i + 1}] ${log.timestamp.toISOString()} [${log.type.toUpperCase()}]`;
                  if (log.source) entry += ` (${log.source})`;
                  entry += `: ${log.text}`;
                  
                  // Add extra details for page errors
                  if (log.source === 'pageerror' && log.stack) {
                    entry += `\n    📍 Location: ${log.filename}:${log.line}:${log.column}`;
