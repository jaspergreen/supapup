...
`);
  ...
  private async navigate(url: string) {
    ...
    // Inject browser agent generator
    const browserScript = fs.readFileSync('./dist/browser-agent-generator.js', 'utf8')
    ...
    console.log(`\nFound ${manifest.elements.length} elements\n`);
    ...
    await this.page!.evaluate((manifestData: any) => {
      (window as any).__AGENT_PAGE__ = {
        manifest: manifestData,
        execute(actionId: string, params: any) {
          ...
          if (action === 'fill' && params?.value !== undefined) {
            (element as any).value = params.value;
  ...
  private async remapPage() {
    ...
    // Re-inject browser agent generator if needed
    try {
      await this.page.evaluate(() => {
        // @ts-ignore
        generateAgentPageInBrowser();
      });
    } catch {
    ...
    await this.page.evaluate((manifestData: any) => {
      if ((window as any).__AGENT_PAGE__) {
        (window as any).__AGENT_PAGE__.manifest = manifestData;
      }
    }, manifest);
  }
